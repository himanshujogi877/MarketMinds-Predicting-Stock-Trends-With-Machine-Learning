...
def train_and_evaluate_model(df):
    ...
    # Ensure X has valid feature names
...
def predict_next_day(reg, df):
    ...
    next_day_features = np.array([[last_row['Date_Ordinal'] + 1, 
                                   ...
                                   last_row['7_day_avg'], 
                                   last_row['30_day_avg'], 
                                   last_row['Sentiment']]])  # Fixed parenthesis
...
def linear_regression_prediction():
    ...
    with st.sidebar:
        ...
        ticker = st.text_input("Enter stock ticker (e.g., 'AAPL', 'GOOG', 'HDFCBANK.IN'):").upper()
...
def lstm_prediction():
    ...
    with st.sidebar:
        ...
        if ticker:
            ...
            start_button = False  # Ensure start_button is defined even if ticker is not entered
    
    # Main content area
    if ticker and start_button:  # Ensure ticker is provided and start_button is clicked
        ...
        # Set default values for LSTM parameters
        time_step = 120  # Default time step (days)
        ...
        if future_date:
            ...
            if future_days > 0:
                ...
                st.write(f"Predicted stock price for {future_date}: {future_predictions[-1]:.2f}")
        ...
        def plot_predictions(data, y_test_actual, test_predict, ticker, scaler, future_predictions=None, future_date=None):
            ...
            predicted_dates = all_dates[-len(test_predict_inverse):]
            predicted_prices = test_predict_inverse
